...
- HTTP 协议：HyperText Transfer Protocol，超文本传输协议，是一个基于 TCP/IP 协议的应用层协议，用于在 Web 浏览器和 Web 服务器之间传递数据
- 客户端与服务器通信传递的数据称为报文，报文分为请求报文和响应报文
- 通信过程:
  - 1. 建立 TCP 连接
  - 2. 客户端向服务器发送请求命令
  - 3. 客户端发送请求头信息
  ...
  - 7. 服务器关闭 TCP 连接
- http 版本
  - 1.0：每次请求建立一次连接，请求完毕后断开连接
  - 1.1：复用连接，一个连接可以发送多个请求,如果长时间没有请求(15mins-30mins)，服务器会断开连接
- 报文结构
  - 请求报文：
    - 请求行：请求方法、请求 URL、协议版本
    - 请求头：键值对，用来说明服务器要使用的附加信息
    - 空行：请求头和请求体之间必须有一个空行
    - 请求体：请求的数据
  - 响应报文：状态行、响应头、空行、响应体
    - 状态行：协议版本、状态码、状态码的描述
    - 响应头：键值对，用来说明服务器要使用的附加信息
    - 空行：请求头和请求体之间必须有一个空行
    - 响应体：响应的数据
